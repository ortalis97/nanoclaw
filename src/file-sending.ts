...
export const EXTENSION_TO_MIME: Record<string, string> = {
  ...
  ts: 'text/typescript', yaml: 'text/yaml', yml: 'text/yaml',
  html: 'text/html', zip: 'application/zip',
  mp4: 'video/mp4', mov: 'video/quicktime',
  mp3: 'audio/mpeg', ogg: 'audio/ogg', wav: 'audio/wav', m4a: 'audio/m4a',
};
...
export function validateContainerPath(containerPath: string): string | null {
  ...
  if (!containerPath.startsWith(prefix)) return 'Path must be inside /workspace/group/';
  const relative = containerPath.slice(prefix.length);
  if (!relative) return 'Path cannot be /workspace/group/ itself';
  if (relative.split('/').some(seg => seg === '..')) return 'Path traversal not allowed';
  return null; // valid
}
...
export function buildBaileysFileContent(
  ...
  return { document: buffer, mimetype, fileName: fileName || 'file', caption } as AnyMessageContent;
}

...
```json
{
  "default": {
    "allow": [
      "972548183824@s.whatsapp.net",
      "972543108007@s.whatsapp.net"
    ],
    "mode": "drop"
  },
  "chats": {
    "972553083455@s.whatsapp.net": {
      "allow": "*",
      "mode": "trigger"
    }
  },
...
- **self-chat** (`972553083455@s.whatsapp.net`): Allow all (`*`) with trigger mode. This is Alfred's own number -- all messages are from "me".
...
ssh -i ~/.ssh/oracle.key ubuntu@158.178.129.108 "mkdir -p ~/.config/nanoclaw"

# 2. Deploy the JSON file
scp -i ~/.ssh/oracle.key ~/.config/nanoclaw/sender-allowlist.json \
  ubuntu@158.178.129.108:~/.config/nanoclaw/sender-allowlist.json

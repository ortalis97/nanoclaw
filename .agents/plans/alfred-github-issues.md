...
**Dependencies**: `GITHUB_TOKEN` (fine-grained PAT with Issues: Read & Write on `ortalis97/nanoclaw`)
...
- [GitHub Fine-Grained PAT](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens#creating-a-fine-grained-personal-access-token)
  - Why: Minimal-scope token (Issues: R/W on `ortalis97/nanoclaw` only) is safest
...
curl -s -X POST \
  ...
  https://api.github.com/repos/ortalis97/nanoclaw/issues \
  -d '{
...
```bash
gh issue list --repo ortalis97/nanoclaw --label alfred --state open --json number,title,state,createdAt
```
```
...
# Scope: Fine-grained PAT, Issues: Read & Write on ortalis97/nanoclaw only
...
```bash
gh label create alfred --color 5319e7 --description "Reported by Alfred" --repo ortalis97/nanoclaw
gh label create bug --color d73a4a --description "Something isn't working" --repo ortalis97/nanoclaw 2>/dev/null || true
gh label create enhancement --color a2eeef --description "New feature or request" --repo ortalis97/nanoclaw 2>/dev/null || true
gh label create todo --color e4e669 --description "Action needed" --repo ortalis97/nanoclaw 2>/dev/null || true
```
...
```bash
ssh -i ~/.ssh/oracle.key ubuntu@158.178.129.108
...
3. Alfred should create an issue — check `github.com/ortalis97/nanoclaw/issues`
4. On Mac: `gh issue list --repo ortalis97/nanoclaw --label alfred --state open` should show it
...
gh issue list --repo ortalis97/nanoclaw --label alfred --state open --json number,title,createdAt
```

...
This is the **fork of NanoClaw** (`ortalis97/nanoclaw`) — a 24/7 WhatsApp bot named Alfred. The source code lives here and the Oracle Cloud Always Free VM (1 OCPU, 1GB RAM, Ubuntu 22.04) runs the deployed version at `/opt/nanoclaw`.

- **Fork:** `git@github.com:ortalis97/nanoclaw.git`
- **Upstream:** `https://github.com/qwibitai/nanoclaw.git`
- **VM:** `ubuntu@158.178.129.108` (SSH key: `~/.ssh/oracle.key`)
...
**SSH to VM:** `ssh -i ~/.ssh/oracle.key ubuntu@158.178.129.108`

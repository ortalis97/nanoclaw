...
| **Bot name** | Alfred |
| **Trigger** | `@Alfred` |
| **VM** | Oracle Cloud Always Free — `ubuntu@158.178.129.108` |
| **App path** | `/opt/nanoclaw` |
| **WhatsApp number** | `+972553083455` (eSIM) |
...
```json
{
  "default": { "allow": ["972548183824@s.whatsapp.net", "972543108007@s.whatsapp.net"], "mode": "drop" },
  "chats": {
...
In this deployment: Or (`972548183824`) and Maya (`972543108007`) are the allowed contacts. DMs from other numbers are silently dropped; groups are active as long as Or or Maya are members.
...
Alfred can file bugs, todos, and observations as GitHub Issues with the `alfred` label on `ortalis97/nanoclaw`. Requires `GITHUB_TOKEN` in `.env` and the `gh` CLI available in the container.

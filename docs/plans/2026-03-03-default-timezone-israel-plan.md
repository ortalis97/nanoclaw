...
**Prerequisites:** SSH access to the VM (`ssh -i ~/.ssh/oracle.key ubuntu@158.178.129.108`). The bot should already be running.
...
> **Exit Criteria:** The `.env.example` change is pushed to `ortalis97/nanoclaw` and the VM has been refreshed.
...
```bash
ssh -i ~/.ssh/oracle.key ubuntu@158.178.129.108 "sudo systemctl status nanoclaw --no-pager"
```
